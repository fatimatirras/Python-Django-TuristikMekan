...
from place.models import Comment, Category
...
class CImagess(models.Model):
    userplaces = models.ForeignKey(UserPlaces, on_delete=models.CASCADE)
    title = models.CharField(max_length=50, blank=True)
    image = models.ImageField(blank=True, upload_to='images/')
...
class UserPlacesForm(ModelForm):
    class Meta:
        model = UserPlaces
...
class UserPlacesImageForm(ModelForm):
    class Meta:
        model = CImages
        fields = ['title', 'image']
